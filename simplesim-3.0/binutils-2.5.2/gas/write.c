...
/*    case O_constant:
      /* Generate a symbol for now.  This actually helps avoid having a
	 local "section symbol" for the absolute section emitted with many
	 files.  Perhaps a better place to fix this would be in
	 adjust_reloc_syms, where that silly symbol is explicitly fetched,
	 but in some instances, for some targets, *some* symbol is
	 required, so I'm not sure if I can eliminate that code.  */
